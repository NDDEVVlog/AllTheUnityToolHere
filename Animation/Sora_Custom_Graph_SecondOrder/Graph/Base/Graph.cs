...
namespace Sora_Ults
{
	...
	public class Graph
	{
		...
		private const float DefaultLineFocusedThickness = 4f;
		...
		public Color HorizontalLineAtOneColor { get; set; } = new Color(1f, 1f, 1f, 0.3f);
		public bool ShouldDrawHorizontalLineAtOne { get; set; } = true;
		#endregion
		...
		public void Draw(Rect position)
		{
			...
			DrawHorizontalLineAtOne(); // Add this line
		...
		private void DrawHorizontalLineAtOne()
		{
			if (!ShouldDrawHorizontalLineAtOne)
			{
				return;
			}

			float yLevelOne = SampleToUIPoint(_logicalDimensions.MinimumX, 1).y;
			Vector2 startPoint = new Vector2(_graphLogicalBottomLeft.x, yLevelOne);
			Vector2 endPoint = new Vector2(_graphLogicalBottomRight.x, yLevelOne);
			Handles.color = HorizontalLineAtOneColor;
